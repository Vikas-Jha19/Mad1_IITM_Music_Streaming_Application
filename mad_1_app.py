...
from mad_1_model import db, User, Admin , Song , Album , Comment , Playlist

app = Flask(__name__)
CORS(app)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///mad_1_music.sqlite3'
db.init_app(app)
api=Api(app)
...
@app.route('/')
def index():
    return render_template('mad_1_index.html')
...
def login():
    ...
    return render_template('mad_1_login.html', registration_message=registration_message, message=message)
...
def admin_login():
    ...
    return render_template('mad_1_admin_login.html', message=message)
@app.route('/user_dashboard/<username>' , methods=["GET","POST"])
def user_dashboard(username):
    user = User.query.filter_by(username=username).first()
    return render_template(
            'mad_1_profile_user_dashboard.html',
            user=user )
...
def creator_dashboard(username):
    ...
    if user and user.role == "creator":
        ...
        return render_template(
            'mad_1_profile_creator_dashboard.html',
            user=user,
...
def edit_song(song_id):
    ...
    if song:
        ...
        return render_template('mad_1_edit_song.html', song=song)
...
def admin_users():
    ...
    users = users.all()
    return render_template('mad_1_user_data.html', users=users, search_query=search_query)
...
def admin_creators():
    ...
    return render_template('mad_1_admin_creators.html', creators=creators, search_query=search_query)
...
def admin_songs():
    ...
    return render_template('mad_1_admin_songs.html', songs=songs)
...
def admin_albums():
    ...
    return render_template('mad_1_admin_albums.html', albums=albums, search_query=search_query)
...
def register():
    ...
    return render_template('mad_1_register.html', message=message)
...
def upload_song(username):
    ...
    return render_template('mad_1_upload_a_song.html', user=user, albums=albums)
...
def create_album(username):
    ...
    return render_template('mad_1_create_album.html', user=user)
...
def play_song(song_id,username):
    ...
    if song:
        return render_template('mad_1_play_song.html', song=song, comments=comments, user=user)
...
def dashboard(username):
    ...
    if user:
        ...
        if user.role == "creator":
            return render_template('mad_1_creator_dashboard.html', user=user, all_songs=songs, playlists=playlists, genres_with_songs_list=genres_with_songs_list, song_search_query=song_search_query, playlist_search_query=playlist_search_query)
        else:
            return render_template('mad_1_user_dashboard.html', user=user, all_songs=songs, playlists=playlists, genres_with_songs_list=genres_with_songs_list, song_search_query=song_search_query, playlist_search_query=playlist_search_query)
...
def confirmation_page(username):
    ...
    if user and request.args.get('confirm') == 'true':
        ...
        return render_template('mad_1_register_as_creator.html', user=user)
...
def create_playlist(username):
    ...
    if user:
        ...
        return render_template('mad_1_create_playlist.html', user=user, all_songs=all_songs)

    return render_template('mad_1_login.html', message={'type': 'danger', 'text': 'User not found.'})
...
def playlist_detail(playlist_id, username):
    ...
    all_songs = Song.query.all()
    if playlist:
        return render_template('mad_1_playlist_details.html', playlist=playlist,all_songs=all_songs,user=user)
...
class SongApi(Resource):
    ...
    def post(self):
        ...
        artist_u=args.get("artist",None)
        lyrics_u=args.get("lyrics",None)
        release_date_u=args.get("release_date",None)
        release_date_u = datetime.strptime(release_date_u, "%Y-%m-%d").date()
        user_id_u=args.get("user_id",None)
        album_id_u=args.get("album_id",None)
        if title_u==None or artist_u==None or lyrics_u==None or release_date_u==None or user_id_u==None or album_id_u==None:
            raise Course_name_error(status_code=400,error_code="SONG_ERROR",error_message="Suffiecient data is not found")
        entry=Song.query.filter_by(title=title_u).first()
        if  entry:
            return "Song already exist",409

        entry=Song(title=title_u,artist=artist_u,lyrics=lyrics_u,release_date=release_date_u,user_id=user_id_u,album_id=album_id_u)
    ...
    def put(self,id):
        ...
        release_date_u=args.get("release_date",None)
        release_date_u = datetime.strptime(release_date_u, "%Y-%m-%d").date()
        if title_u==None or artist_u==None or lyrics_u==None or release_date_u==None:
            raise Course_name_error(status_code=400,error_code="SONG_ERROR",error_message="Suffiecient data is not found")
        if not entry:
            raise NotFoundError(status_code=404)
        
        entry.title=title_u
        entry.artist=artist_u
        entry.lyrics=lyrics_u
        entry.release_date=release_date_u
        db.session.commit()
        jsonobj={
                "song_id": entry.id,
                "title": entry.title,
...
class AlbumApi(Resource):
    ...
    def post(self):
        args=parser.parse_args()
        name_u=args.get("name",None)
        genre_u=args.get("genre",None)
        artist_u=args.get("artist",None)
        user_id_u=args.get("user_id",None)

        if name_u==None or artist_u==None or genre_u==None or user_id_u==None:
            raise Course_name_error(status_code=400,error_code="ALBUM_ERROR",error_message="Suffiecient data is not found")
        entry=Album.query.filter_by(name=name_u).first()
        if  entry:
            return "Album already exist",409

        entry=Album(name=name_u,artist=artist_u,genre=genre_u,user_id=user_id_u)
        db.session.add(entry)
        db.session.commit()
        jsonobj={
                "album_id": entry.id,
    ...
    def put(self,id):
        ...
        genre_u=args.get("genre",None)
        artist_u=args.get("artist",None)
        if name_u==None or artist_u==None or genre_u==None:
            raise Course_name_error(status_code=400,error_code="ALBUM_ERROR",error_message="Suffiecient data is not found")
        if not entry:
            raise NotFoundError(status_code=404)
